import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { of } from 'rxjs';

import { TreatyService } from '../service/treaty.service';

import { TreatyComponent } from './treaty.component';

describe('Treaty Management Component', () => {
  let comp: TreatyComponent;
  let fixture: ComponentFixture<TreatyComponent>;
  let service: TreatyService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      declarations: [TreatyComponent],
    })
      .overrideTemplate(TreatyComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(TreatyComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(TreatyService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 'ABC' }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.treaties?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });
});
